...
if __name__ == "__main__":
    INPUT_PATH="/home/sounak/Images/ISIC2018_Task3_Training_GroundTruth/ISIC2018_Task3_Training_GroundTruth.csv"
    dataset=pd.read_csv(INPUT_PATH)
    dataset=np.asarray(dataset)
    dataset=dataset[:, 1:]
    y_train=np.argmax(dataset, axis=-1)
    ...
    model = SEMobileNet(alpha=1.0,depth_multiplier=1, dropout=1e-3, include_top=True, pooling='max', classes=7)
    ...
    checkpointer = ModelCheckpoint('mobilenet_best.h5', verbose=1, save_best_only=True)
    model.summary()
    model.fit_generator(batches, steps_per_epoch=num_train_steps, epochs=10, callbacks=[early_stopping, checkpointer], validation_data=val_batches, validation_steps=num_valid_steps, class_weight=class_weights)
